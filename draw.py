...
def draw_Idic(df, ind1, ind2):
     fig = go.Figure(data=[go.Candlestick(x=[i for i in range(len(df))],
                ...
                low=df['Low'],
                close=df['Close'])])
     fig.add_trace(go.Scatter(x = list(range(len(df))), y = ind1, mode="lines", line=go.scatter.Line(color="orange"), color="aroon", labels={"aroon": "up"}))
     fig.add_trace(go.Scatter(x = list(range(len(df))), y = ind2, mode="lines", line=go.scatter.Line(color="blue"), color="aroon", labels={"aroon": "down"}))
     fig.show()
